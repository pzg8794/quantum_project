...
class MultiRunEvaluator:
    ...
    def __init__(self, configs=None, runs=1, base_frames=4000, frame_step=2000, 
                 attack_type="stochastic", models=None, scenarios=None, attack_intensity=1.0, base_seed=12345, enable_progress=False):
        """
        ...
        print(f"Frame Range: {base_frames} -> {base_frames + (self.runs-1)*frame_step} (step: {frame_step})")
    ...
    def run_experiment(self, exp_no, offset=100, models=None, attack_category="Stochastic", attack_rate=0.25):
        ...
        # **ASK THE ALLOCATOR for qubit allocation**
        # (Pass route_stats if available, else it uses defaults)
        route_stats = {}  # Could be populated from previous experiment results
        qubit_cap = self.configs.allocator.allocate(
            timestep=exp_no,
            route_stats=route_stats
        )

        # NOW set environment params with dynamically allocated qubits
        self.configs.set_environment(
            qubit_cap=qubit_cap,  # ← From allocator, not hardcoded!
        ...
        # Run experiment
        try:
            experiment_results = runner.run_experiment(frame_count=frame_count, models=self.models)
            ...
            print(f"Experiment {exp_id} completed successfully in environment {str(self.environment)}")
    ...
    def run_experiments(self, runs=None, attack_type=None, models=None):
        ...
        """
        if runs: self.runs = runs
        if models: self.models = models
        if attack_type: self.attack_type = attack_type
        if self.attack_type not in self.env_experiments: self.env_experiments[self.attack_type] = {}
    ...
    def run_scenario_model_evaluation(self, runs=None, models=None, attack_type=None):
        ...
        print(f"TESTING ENVIRONMENT SCENARIO: {self.test_scenarios[self.attack_type].upper()}")
        print("="*50)
    ...
    def run_scenarios_model_evaluation(self, runs=None, models=None, env_type=None, scenarios=None, cal_winner=True):
        """
        ...
        if runs: self.runs = runs
        if models: self.models = models
        if env_type: self.env_type = env_type
        if scenarios: self.test_scenarios = scenarios
        if cal_winner: self.cal_winner = cal_winner

        print(f"Models to Test:             \t{', '.join(self.models)}")
        print(f"Test Scenarios:             \t{', '.join(self.test_scenarios.values())}")
        print(f"Experiments per Scenario:   \t{self.runs}")
        print("="*70)

        # Run experiments for all specified scenarios
        self.evaluation_results = {}
        for attack_type in self.test_scenarios.keys():
        ...
        if self.cal_winner: self.calculate_scenarios_winner(self.evaluation_results)
    ...
    # Usage functions
    def test_stochastic_environment(self, runs=1, models=None, scenarios=None, env_type='stochastic', cal_winner=True):
        """
