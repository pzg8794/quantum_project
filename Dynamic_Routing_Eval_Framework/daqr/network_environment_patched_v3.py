...
class QuantumEnvironment:
    ...
    def __init__(self, attack, qubit_capacities=(8, 10, 8, 9),
        ...
        # ✅ FIX 1: Normalize qubit_capacities input EARLY
        ...
        # ✅ FIX 2: Use allocator with normalization
        if self.allocator:
            initial_stats = {
                i: {'success_rate': 0.5, 'pulls': 0, 'successes': 0, 'failures': 0}
                for i in range(len(self.qubit_capacities))
            }
            allocator_result = self.allocator.allocate(
                timestep=0, route_stats=initial_stats, verbose=False
            )
            
            # ✅ NORMALIZE allocator result
            ...
            # ✅ REGENERATE contexts with new capacities
            self.contexts = self._generate_contexts()
        else:
            self.qubit_capacities = self._normalize_qubit_capacities(qubit_capacities)
            print(f"📌 Static Allocation: {self.qubit_capacities}")

        # Update num_paths based on actual qubit_capacities
        ...
        try:
            for path_idx in range(self.num_paths):
                ...
                if path_idx >= len(self.contexts) or not self.contexts[path_idx]:
                    ...
                    # Compute reward for each context (supports multiple fidelity calculator APIs)
                    for context in self.contexts[path_idx]:
                        try:
                            # Newer calculators (e.g., FullPaper2FidelityCalculator)
                            ...
                            # Backward-compatible calculators (e.g., CascadedFidelityCalculator)
                            fidelity = self.fidelity_calculator.compute_path_fidelity(
                        ...
                        # If a reward function is provided, use it; otherwise fidelity is the reward.
                        if getattr(self, "reward_function", None) is not None:
                            rf = self.reward_function
                            if hasattr(rf, "compute_reward"):
                                reward = rf.compute_reward(fidelity)
                            else:
                                reward = rf.compute(fidelity) if hasattr(rf, "compute") else float(fidelity)
                        else:
