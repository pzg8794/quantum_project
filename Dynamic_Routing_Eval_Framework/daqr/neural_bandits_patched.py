...
class EXPNeuralUCB(QuantumModel):
    ...
    def _calculate_group_probabilities(self):
        """Numerically-stable EXP3 group probability computation.

        The previous implementation could overflow (exp -> inf) and produce NaNs,
        which then triggers NumPy's: "probabilities are not non-negative".
        """

        # Raw EXP3 scores
        scores = np.array([
            self.eta * float(sum(self.estimate_group_reward[g]))
            for g in range(self.num_groups)
        ], dtype=float)

        # Stabilize softmax: exp(scores - max(scores))
        ...
        if not np.isfinite(denom) or denom <= 0.0:
            # Fallback to uniform if something went sideways
            return np.ones(self.num_groups, dtype=float) / float(self.num_groups)

        soft = exp_shifted / denom
        prob_array = (self.gamma / float(self.num_groups)) + (1.0 - self.gamma) * soft

        # Final guardrails
        prob_array = np.clip(prob_array, 0.0, np.inf)
        s = float(np.sum(prob_array))
        if s <= 0.0 or not np.isfinite(s):
            return np.ones(self.num_groups, dtype=float) / float(self.num_groups)

        return (prob_array / s).astype(float)
        ...
        if self.mode in ['hybrid', 'exp3']:
            for group_index in range(self.num_groups):
                if group_index == selected_path:
                    ...
                    # Importance-weighted reward can explode when safe_p is tiny. That can
                    # overflow EXP3's exponentials, producing NaNs and invalid probabilities.
                    iw = observed_reward / safe_p
                    if not math.isfinite(iw):
                        iw = 0.0
                    # Conservative cap to keep weights numerically stable.
                    iw = max(min(iw, 1e6), -1e6)
                    self.estimate_group_reward[group_index].append(iw)
                else:
