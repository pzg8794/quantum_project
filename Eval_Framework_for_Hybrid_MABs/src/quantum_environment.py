...
class QuantumEnvironment:
    ...
    def __init__(self, qubit_capacities=(8, 10, 8, 9), frame_length=4000, seed: int | None = None):
        ...
        # Precomputed contexts and reward tables
    ...
    """
    Base Quantum Network Environment.
    Handles: network topology, contexts, and CONSISTENT reward tables.
    """
    def __init__(self, qubit_capacities=(8, 10, 8, 9), frame_length=4000, seed: int | None = None, entanglement_success_factor: int = 3000):
        """
        *** CORRECTED INITIALIZER ***
        Added 'entanglement_success_factor' to decouple reward calculation
        from the experiment's frame_length.
        """
        self.qubit_capacities = tuple(qubit_capacities)
        self.num_paths = len(self.qubit_capacities)
        self.frame_length = int(frame_length)
