...
class QuantumExperimentRunner:
    ...
    leverages the QuantumExperimentConfig factory to set up and execute tests
    across various scenarios and models.
    """
    def __init__(self, config: QuantumExperimentConfig | None = None, base_seed=12345, attack_type="markov", attack_intensity=1.0, enable_progress=True, use_locks=False):
        self.configs = config if config is not None else QuantumExperimentConfig()
    ...
    def setup_environment(self, frame_count: int, qubit_cap: tuple, model_seed_offset: int):
        """
        **CORRECTED FACTORY USAGE**: Configures and creates the appropriate
        ...
        # 2. Tell the config object WHAT attack scenario we want.
        self.configs.set_attack_scenario(
            attack_type=self.attack_type,
            attack_rate=0.25, # Pass any necessary kwargs for the strategy
            attack_intensity=self.attack_intensity
        )
        
        # 3. Tell the config object the CORE parameters for the environment.
        self.configs.set_environment_params(
            qubit_cap=qubit_cap,
            frames_no=frame_count,
            seed=self.experiment_seed
        )

        # 4. Ask the factory to BUILD and return the correct environment object.
    ...
    def run_algorithm(self, algorithm_name: str, frame_count: int = 4000, qubit_cap: tuple = (8, 10, 8, 9)):
        """
        ...
        # 1. A SINGLE, CORRECT call to setup_environment.
        # It is called only ONCE, at the beginning, with the correct, unique
        # seed_offset for the specific algorithm being run.
        self.setup_environment(
            frame_count=frame_count, 
            qubit_cap=qubit_cap, 
            model_seed_offset=config['seed_offset']
        )
        
        # 2. The rest of the method can now proceed with the certainty
        #    that self.environment is the correct one.
        ...
        try:
            ...
            while total_reward <= 0.0:
                ...
                try:
                    ...
                    not_oracle = algorithm_name != 'Oracle' 
                    # over_valid = not_oracle and total_reward > 98
                    not_valid = not_oracle and total_reward < model._get_expected_min_reward(algorithm_name)
                    
                    if not_valid: 
                        curr_reward = total_reward
                        total_reward = 0.0
                        raise ValueError(f"Invalid reward '{curr_reward}' retrying")
