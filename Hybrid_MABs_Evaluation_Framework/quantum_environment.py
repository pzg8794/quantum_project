...
class QuantumEnvironment:
    ...
    def _print_initialization(self):
        ...
        print(f"| Frame Length: \t| {str(self.frame_length):<15}    |")
        print("=" * 50)
    ...
    def cleanup(self, verbose=False):
        """
        Clean up base environment resources.
        
        This method is inherited by all environment subclasses.
        ...
        # 1. Large numpy arrays
        for attr in ['contexts', 'reward_list', 'oracle_rewards', 
                     'path_rewards', 'optimal_path_history', 'optimal_action_history']:
            if hasattr(self, attr):
                delattr(self, attr)
                cleanup_items.append(attr)
        
        # 2. RNG cleanup (release state)
        if hasattr(self, 'rng'):
            del self.rng
            cleanup_items.append("rng")
        ...
        if verbose:
            print(f"✓ {self.__class__.__name__} cleaned: {', '.join(cleanup_items)}")
...
class AdversarialQuantumEnvironment(QuantumEnvironment):
    ...
    def __init__(self, qubit_capacities=(8, 10, 8, 9), frame_length=4000,
                 attack: AttackStrategy | None = None, seed: int | None = None):
        ...
        # generate ONCE per env to avoid "rerolling" on each call
        # Pre-generate once per env, cast to compact type for consistency
        self.attack_pattern = self.attack.generate(self.rng, self.frame_length, self.num_paths).astype(np.int8, copy=False)
    ...
    def cleanup(self, verbose=False):
        """
        Extended cleanup for adversarial environment.
        
        Cleans up attack-specific resources, then calls parent cleanup.
        """
        cleanup_items = []
        
        # 1. Attack strategy cleanup
        if hasattr(self, 'attack') and self.attack is not None:
            if hasattr(self.attack, 'cleanup'):
                self.attack.cleanup(verbose=verbose)
            del self.attack
            cleanup_items.append("attack_strategy")
        
        # 2. Attack pattern (large array)
        if hasattr(self, 'attack_pattern'):
            del self.attack_pattern
            cleanup_items.append("attack_pattern")
        
        if verbose and cleanup_items:
            print(f"✓ AdversarialQuantumEnvironment specific cleanup: {', '.join(cleanup_items)}")
        
        # 3. Call parent cleanup for base resources
        super().cleanup(verbose=verbose)