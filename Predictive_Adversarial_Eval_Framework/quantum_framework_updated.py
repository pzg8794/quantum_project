...
class MultiRunEvaluator:
    ...
    def run_experiments(self, attack_type=None, num_experiments=3, algorithms=["EXPNeuralUCB", 'GNeuralUCB', 'EXPUCB', 'Oracle'], test_scenarios={'stochastic': 'Stochastic (Natural Random Failures)', 'adaptive': 'Adversarial (Strategic Reactive Attacks)'}):
        """
    ...
    def run_stochastic_vs_adversarial_comparison(self, runs=1, models=["EXPNeuralUCB", 'GNeuralUCB', 'EXPUCB', 'Oracle'], selected_model="EXPNeuralUCB", test_scenarios={'stochastic': 'Stochastic (Natural Random Failures)', 'adaptive': 'Adversarial (Strategic Reactive Attacks)'}):
        """
        ...
        for attack_type, description in test_scenarios.items():
            ...
            experiment_results = self.run_experiments(num_experiments=runs, attack_type=attack_type,algorithms=models, test_scenarios=test_scenarios)
        ...
        exp_stochastic = stochastic_data['results'][selected_model]['final_reward']
        exp_adversarial = adversarial_data['results'][selected_model]['final_reward']

        robustness_loss = ((exp_stochastic - exp_adversarial) / exp_stochastic * 100) if exp_stochastic > 0 else 0

        print(f"📊 {selected_model} Robustness Analysis:")
...
# Usage functions
def test_stochastic_vs_adversarial(runs=1, base_frames=4000, frame_step=2000, models=["EXPNeuralUCB", 'GNeuralUCB', 'EXPUCB', 'Oracle'], selected_model="EXPNeuralUCB", test_scenarios={'stochastic': 'Stochastic (Natural Random Failures)', 'adaptive': 'Adversarial (Strategic Reactive Attacks)'}):
    """
    ...
    # Run the comparison
    results = evaluator.run_stochastic_vs_adversarial_comparison(runs=runs, models=models, selected_model=selected_model, test_scenarios=test_scenarios)
