...
class CEXPNeuralUCB(EXPNeuralUCB):
    ...
    Modes:
    - 'cmab': CMAB(EXP4) + Neural UCB  
    - All other modes inherited from EXPNeuralUCB unchanged
    """
    
    def __init__(self, X_n, reward_list, frame_number, mode='cmab', 
                 gamma_factor=0.1, eta_factor=0.005, beta=0.2, n_experts=4):
        # Call parent constructor
        super().__init__(X_n, reward_list, frame_number, mode, 
                         gamma_factor, eta_factor, beta)
        
        # CMAB-specific parameters
        self.n_experts = n_experts
        self.n_features = len(X_n[0]) if X_n else 2  # Context features
        
        # Override mode-specific initialization if needed
        if mode == 'cmab':
            self._initialize_cmab_components()
    
    def _initialize_cmab_components(self):
        """Initialize CMAB(EXP4) instead of EXP3"""
        # Replace EXP3 with CMAB
        self.cmab = CMAB(
            bandit="exp4",
        ...
        # Track expert advice for EXP4
        self.expert_advice_history = []
        
    def select_group(self, frame):
        """Override group selection to use CMAB instead of EXP3"""
        if self.mode == 'cmab':
            return self._select_group_cmab(frame)
        else:
            return super().select_group(frame)  # Use parent's method
    
    def _select_group_cmab(self, frame):
        """CMAB(EXP4) group selection"""
        # Generate expert advice (could be based on context, history, etc.)
        expert_advice = self._generate_expert_advice(frame)
        
        # Use CMAB to pick arm (group)
        selected_group = self.cmab.pickArm(advice=expert_advice)
        
        # Store for tracking
        self.expert_advice_history.append(expert_advice)
        
        return selected_group, expert_advice  # Return advice for update
    
    def _generate_expert_advice(self, frame):
        """Generate expert advice matrix for EXP4"""
        # Simple expert advice based on path characteristics
        advice = np.zeros((self.n_experts, self.num_groups))
        
        # Expert 1: Prefer path 0 (shortest)
        advice[0, 0] = 1.0
        
        # Expert 2: Prefer path with highest historical reward
        if hasattr(self, 'group_rewards'):
            best_group = np.argmax(self.group_rewards)
            advice[1, best_group] = 1.0
        else:
            advice[1, :] = 0.25  # Uniform initially
            
        # Expert 3: Random exploration
        advice[2, :] = np.random.dirichlet([1] * self.num_groups)
        
        # Expert 4: UCB-style (prefer less explored + high reward)
        for i in range(self.num_groups):
            if hasattr(self, 'group_counts') and self.group_counts[i] > 0:
                avg_reward = self.group_rewards[i] / self.group_counts[i]
                exploration = np.sqrt(2 * np.log(frame + 1) / self.group_counts[i])
                advice[3, i] = avg_reward + exploration
            else:
                advice[3, i] = 1.0  # High value for unexplored
        
        # Normalize expert 4
        if advice[3, :].sum() > 0:
            advice[3, :] = advice[3, :] / advice[3, :].sum()
        
        return advice
    
    def update_group_selection(self, selected_path, observed_reward, advice=None):
        """Override group update to use CMAB instead of EXP3"""
        if self.mode == 'cmab':
            self._update_group_cmab(selected_path, observed_reward)
        else:
            super().update_group_selection(selected_path, observed_reward, advice)
    
    def _update_group_cmab(self, selected_path, observed_reward):
        """Update CMAB with observed reward"""
        self.cmab.update(observed_reward)
        
        # Track group statistics for expert advice generation
        if not hasattr(self, 'group_rewards'):
            self.group_rewards = [0.0] * self.num_groups
...
class CEXPNeuralUCB(EXPNeuralUCB):
    ...
    def __init__(self, X_n, reward_list, frame_number, mode='cmab', 
        ...
        super().__init__(X_n, reward_list, frame_number, 'neural', 
                         gamma_factor, eta_factor, beta)
